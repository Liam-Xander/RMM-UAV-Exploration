...
public:
  ...
  Eigen::Vector3i min_cut_expand;
  Eigen::Vector3i max_cut_expand;
  // ros::Publisher map_pub_predict;
  void publishPredictCloud(const pcl::PointCloud<pcl::PointXYZ>& cloud);
  // Expose these member variables to allow SDFMap to access them
  std::string frame_id_;
  ros::Publisher map_pub_predict;
private:
  // Add hash function for grid indices
  struct HashVector3i {
  ...
  pcl::PointCloud<pcl::PointXYZ> intersectionpoints(
    const pcl::PointCloud<pcl::PointXYZ>& cloudMap,
    const Eigen::Vector3i& min_cut_expand,
    const Eigen::Vector3i& max_cut_expand);
  pcl::PointCloud<pcl::PointXYZ> intersectionpoints_surface(
    const pcl::PointCloud<pcl::PointXYZ>& cloudMap,
    const Eigen::Vector3i& min_cut_expand,
    const Eigen::Vector3i& max_cut_expand);
  pcl::PointCloud<pcl::PointXYZ> intersectionpoints_surface_new(
    const pcl::PointCloud<pcl::PointXYZ>& cloudMap,
    const Eigen::Vector3i& min_cut_expand,
    const Eigen::Vector3i& max_cut_expand);

  void rcvGlobalPointCloudCallBack(const sensor_msgs::PointCloud2ConstPtr& msg);
  ...
  void publishMapAll();
  // bool getParams(ros::NodeHandle& nh, Eigen::Vector2d& mmin, Eigen::Vector2d& mmax);
  ...
  ros::Publisher map_local_pub_, map_local_inflate_pub_, esdf_pub_, map_all_pub_, unknown_pub_,
      update_range_pub_, depth_pub_, map_pub_boundary_expand, 
      // map_pub_predict, 
  intersection_cloud_pub;
  ...
  double x_min, x_max, y_min, y_max;//added
  ...
  // unique_ptr<SDFMap> ms_;
  std::unique_ptr<SDFMap> ms_;
